...
class InventoryUpdatesSystemStack(Stack):
  ...
  def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
      ...
      # Create the Dead Letter Queue (DLQ)
       ...
       # Create the DynamoDB table
        ...
        sfn_consume_sqs_message = sfn.StateMachine(self, "state-machine",
            definition_body=sfn.DefinitionBody.from_file("./product_stepfn.asl.json"),
    ...
    # Define event bridge pipeline with sqs queue as source and step function as target
        ...
        order_stepfn = sfn.StateMachine(self, "order-state-machine",
            definition_body=sfn.DefinitionBody.from_file("./order_stepfn.asl.json"),
            state_machine_type = sfn.StateMachineType.EXPRESS,
            state_machine_name = "InventoryOrderStateMachine",
            logs = sfn.LogOptions(
                destination=logs.LogGroup(self, "order-state-machine-logs_inventory_updates"),
                level=sfn.LogLevel.ALL,
                include_execution_data=True
            )
        )  
        order_stepfn.grant_start_execution(crud_stepfn)
        Tags.of(order_stepfn).add("department", "inventory")
        table.grant_read_write_data(order_stepfn)
        order_stepfn.add_to_role_policy(iam.PolicyStatement(
            effect=iam.Effect.ALLOW,
        ...
        # Orders
        orders = inventory_api.root.add_resource("orders")
        place_order = inventory_api.root.add_resource("order")
        order = place_order.add_resource("{order_id}")
